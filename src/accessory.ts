  ...
  createListener() {
    ...
    client.on('data', data => {
      try {
        ...
        if (message.type === 'NotifyStatus' && message.success === true) {
          this.log.debug('Received status notification:', message);
    ...
    client.on('error', error => {
      this.log.error(`Listener error: ${error.message}`);
      setTimeout(() => this.createListener(), 5000);
    });

    client.on('close', () => {
      this.log.warn('Listener connection closed');
      setTimeout(() => this.createListener(), 5000);
    });
  }

  registerHandlers() {
    this.log.debug('Registering handlers');
